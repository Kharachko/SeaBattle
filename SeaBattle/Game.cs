...
public class Game
{
    ...
    public GameField PlayerField => _playerField;
    public GameField EnemyField => _enemyField;
    private Random _random;
    private GameSettings _settings;
    private GameFieldFactory _factory;

    public Game(GameSettings settings, GameFieldFactory factory)
    {
        _settings = settings;
        _factory = factory;
    ...
    public void Play()
    {
        while (!_playerField.AllShipsSunk() && !_enemyField.AllShipsSunk())
        {
            ...
            {
                if (x >= 0 && x < _settings.FieldSize && y >= 0 && y < _settings.FieldSize)
                {
